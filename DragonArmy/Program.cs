namespace DragonArmy
{
    ...
    class Program
    {
        ...
        {
            // color         name    data  
            Dictionary<string, Dictionary<string, double[]>> dragoons =
            ...
            foreach (var color in dragoonsByColor)
            {
                ...
                Console.WriteLine("{0}{1:f2}{2:f2}{3:f2}", color.Key, averDamage, averHealth, averArmor);
                foreach (var dragoon in dragoonsByName)
                {
                    Console.WriteLine(
                        "-{0}->damage:{1:f2},health:{2:f2},armor:{3:f2}", 
                        dragoon.Key, 
                        dragoon.Value[0], 
                        dragoon.Value[1], 
                        dragoon.Value[2]);
